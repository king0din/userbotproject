...
def register(bot):
    ...
    @check_ban
    async def pdefaults_handler(event):
        user_id = event.sender_id
        u = await _ensure_logged(event)
        if not u:
            return
        await event.answer("⭐ Varsayılanlar yükleniyor...")
        client = await smart_session_manager.get_or_create_client(user_id)
        if not client:
            await event.answer("❌ Bağlantı kurulamadı, tekrar giriş yap", alert=True)
            return
        try:
            count = await plugin_manager.activate_default_plugins(user_id, client)
        except Exception:
            log.warning("Varsayılanlar yüklenemedi (user=%s)", user_id, exc_info=True)
            count = 0
        text, rows = await build_plugins_page(user_id, 0)
        await _render(event, f"⭐ {count} varsayılan plugin yüklendi.\n\n" + text, rows)
